...
def to_cyrillic(text):
    ...
    1. ye = е in the beginning of a words or after a vowel
    2. e = э in the beginning of a words or after a vowel
    3. ц exception words
    4. э exception words
    """
    ...
    def replace_soft_sign_words(m):
        words = m.group(1)
        if words.isupper():
            result = SOFT_SIGN_WORDS[words.lower()].upper()
        elif words[0].isupper():
            result = SOFT_SIGN_WORDS[words.lower()]
...
def has_cyrillic(text):
    return bool(search('[\u0400-\u04FF]', text))


def transliterate(text):
    return to_cyrillic(text) if not has_cyrillic(text) else to_latin(text)
